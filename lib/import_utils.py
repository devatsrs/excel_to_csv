...
def expand_headers(headers, data_offset, rows):
    ...
    row_length = max(itertools.chain([len(headers)],
                                     (_count_nonempty(r) for r in itertools.islice(rows, data_offset,
                                                                                   None))))
    header_values = [h.value.strip() for h in headers] + \
        [u''] * (row_length - len(headers))
...
def like_header_col(row):
    ...
    search = [ "product", "description", "item", "msrp", "map", "part", "price", "cost", "model", "type"]
    log.debug(f"len(row) = {len(row)}")
    for row_text in row:
        row_text = prepar_header_col(row_text)
        words = row_text.split(" ")
        if any(prepar_header_col(word) in search for word in words):
            return True
